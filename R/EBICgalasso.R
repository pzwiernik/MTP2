...
#' 
##### Algorithm 3
EBICgalasso <- function(S,n=NULL,L= NULL,U=NULL,tol=1e-6,edge.tol=1e-4,gamma=0.5,rhomin=0.01,rhomax=1,nrhos=50,output=TRUE){
  ...
  for (rho in rhos){
    LL <- rho*L
    UU <- rho*U
    res <- galasso(S,L=LL,U=UU,output=FALSE)
    # compute EBIC
    K <- res$K
